            ...
            Data.findById(_id, function (err, data) {
                if (err) return resource.setResponse(res, error, next);
                    ...
                    switch (data.name) {
                        ...
                        case "lastAccess":
                            http.request({
                                method: 'get', port: process.env.CURRENT_PORT | 3000, path: '/users/last',
                                headers: {
                        ...
                        case "tweets":
                            ...
                            http.request({
                                method: 'get', port: process.env.CURRENT_PORT | 3000, path: '/users',
                                headers: {
                                    'Content-type': 'application/json',
                                    'Authorization': 'Bearer ' + jwt.sign(req.payload, process.env.MY_SECRET)
                                }
                        ...
                        case "tweetsxuser":
                            http.request({
                                method: 'get', port: process.env.CURRENT_PORT | 3000, path: '/users/tweets',
                                headers: {
                                    'Content-type': 'application/json',
                                    'Authorization': 'Bearer ' + jwt.sign(req.payload, process.env.MY_SECRET)
                                }
